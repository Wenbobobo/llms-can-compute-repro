...
def build_checklist_rows(inputs: dict[str, Any]) -> list[dict[str, object]]:
    ...
    return [
        {
            "item_id": "h45_docs_authorize_r47_and_keep_f22_r48_blocked",
            "status": "pass"
            if contains_all(
            ...
            and contains_all(
                ...
                [
                    "saved minimal blocked comparator-only bundle after positive `r46` and executed `h45`",
                    "it does not authorize model execution",
                    "`h45` selected `authorize_r47_origin_restricted_frontend_translation_gate`",
                ],
            )
            and contains_all(
                inputs["f22_status_text"],
                [
                    "saved blocked comparator-only bundle after completed `h45`",
                    "stays blocked behind exact `r47_origin_restricted_frontend_translation_gate` plus later explicit `h46_post_r47_frontend_bridge_decision_packet`",
                ],
            )
            and contains_all(
                inputs["r47_readme_text"],
                [
                    "authorized next exact frontend bridge gate after landed `r46` and executed `h45`",
            ...
            "notes": "H45 should select exactly R47 while keeping F22 blocked and R48 deferred behind later explicit H46.",
        },
        {
        ...
        {
            "item_id": "shared_control_surfaces_make_h45_current_with_r47_next",
            "status": "pass"
            if contains_all(
                inputs["readme_text"],
                [
                    ...
                    "`r47` is now the next exact runtime candidate",
                    "`f22` is saved only as a blocked comparator bundle",
                ],
            )
            and contains_all(
                inputs["status_text"],
                [
                    "`h45_post_r46_surface_decision_packet`, not the preserved prior `h44` packet",
                    "`r47_origin_restricted_frontend_translation_gate`",
                    "`f22_post_r46_useful_case_model_bridge_bundle` only as a blocked future comparator bundle",
                ],
            )
            and contains_all(
            ...
            and contains_all(
                ...
                [
                    ...
                    "../milestones/h45_post_r46_surface_decision_packet/",
                    "../milestones/f22_post_r46_useful_case_model_bridge_bundle/",
            ...
            and contains_all(
                ...
                [
                    ...
                    "`r47_origin_restricted_frontend_translation_gate` is now the next required exact runtime lane",
                    "`f22_post_r46_useful_case_model_bridge_bundle` is saved only as a blocked future comparator bundle",
                ],
            )
            and contains_all(
                ...
                [
                    "post-`r46` `h45` surface-decision wave",
                    "new `scripts/export_h45_post_r46_surface_decision_packet.py`",
                    "new `results/h45_post_r46_surface_decision_packet/summary.json`",
                ],
            )
            else "blocked",
            "notes": "Shared control surfaces should make H45 current, keep H43 as the paper-grade endpoint, and fix R47 as the next exact runtime lane.",
        },
    ]
...
def build_snapshot(inputs: dict[str, Any]) -> list[dict[str, object]]:
    lookup = {
        ...
        "docs/milestones/F22_post_r46_useful_case_model_bridge_bundle/README.md": (
            "f22_readme_text",
            ["blocked future comparator bundle", "does not authorize model execution"],
        ),
        ...
        "docs/publication_record/current_stage_driver.md": (
            ...
            ["h45_post_r46_surface_decision_packet", "r47_origin_restricted_frontend_translation_gate"],
        ),
        "tmp/active_wave_plan.md": (
            "active_wave_plan_text",
            ["`h45_post_r46_surface_decision_packet` is the current active docs-only", "`f22_post_r46_useful_case_model_bridge_bundle` is saved only as a blocked future comparator bundle"],
        ),
    }
