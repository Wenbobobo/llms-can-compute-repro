...
def evaluate_scorer(
    ...
    for sample in samples:
        ...
        bucket_state["sample_count"] = int(bucket_state["sample_count"]) + 1 # type: ignore
        bucket_state["sample_correct"] = int(bucket_state["sample_correct"]) + int(correct) # type: ignore

        programs: dict[str, list[bool]] = bucket_state["programs"] # type: ignore
